...
/** Puts the current thread to sleep.  It will not be scheduled
   again until reached the time to wake up.
...
void thread_sleep(int64_t ticks)
{
  ...
  current->wakeup_timestamp = timer_ticks() + ticks;
  list_push_back(&sleep_list, &current->sleepelem);
  thread_block();
  intr_set_level(old_level);
}

/** Try to wakeup sleeping threads */
void thread_try_wakeup()
{
  ...
  for (e = list_begin(&sleep_list); e != list_end(&sleep_list);)
  {
    ...
    if (t->wakeup_timestamp == current_ticks)
    {
      thread_unblock(t);
      e = list_remove(e);
    }
